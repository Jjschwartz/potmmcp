...
class PEB0(M.Belief):
    ...
    def sample(self) -> M.State:
        if self._evader_start_coord is None:
            evader_start_coord = self._rng.choice(self._grid.evader_start_coords)
        else:
            evader_start_coord = self._evader_start_coord

        if self._pursuer_start_coord is None:
            pursuer_start_coord = self._rng.choice(self._grid.pursuer_start_coords)
        else:
        ...
        return PEState(
            ...
            pursuer_start_coord,
            goal_coord,
            self._grid.get_shortest_path_distance(evader_start_coord, goal_coord),
        )
...
class PursuitEvasionModel(M.POSGModel):
    ...
    def action_spaces(self) -> Tuple[spaces.Space, ...]:
        return tuple(spaces.Discrete(len(Direction)) for _ in range(self.n_agents))

    @property
    def observation_spaces(self) -> Tuple[spaces.Space, ...]:
        # Tuple[WallObs, seen , heard]
        env_obs_space = spaces.Tuple(tuple(spaces.Discrete(2) for _ in range(6)))
    ...
    def get_agent_initial_belief(self, agent_id: int, obs: M.Observation) -> M.Belief:
        ...
        return PEB0(
            self.grid, self._rng, evader_start_coord=obs[1], pursuer_start_coord=obs[2]
        )
    ...
    def _get_next_state(self, state: PEState, actions: PEJointAction) -> PEState:
    ...
    def _get_opponent_heard(
        self, ego_coord: grid_lib.Coord, opp_coord: grid_lib.Coord
    ) -> bool:
