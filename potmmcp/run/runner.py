...
from potmmcp.policy import BasePolicy
...
def run_episode_loop(
    ...
    joint_timestep = (  # type: ignore
        joint_obs,
        tuple(0.0 for _ in range(env.n_agents)),
        False,
        {},
    )
    init_action = None
    ...
    while not episode_end:
        ...
        yield EpisodeLoopStep(env, joint_timestep, joint_action, policies, episode_end)
...
def run_episodes(
    ...
    while episode_num < num_episodes and not time_limit_reached:
        ...
        timestep_sequence = run_episode_loop(env, policies, episode_step_limit)
