...
from potmmcp.run.exp import ExpParams, PolicyParams
...
def get_exp_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "~/potmmcp_results/<env_id>/ dir as root results dir."
        ),
    )
    return parser
...
def get_pairwise_exp_params(
    ...
    renderer_fn: Optional[Callable] = None,
    ...
    **kwargs
    ...
    for i, (exp_seed, *policies) in enumerate(
        product(range(num_seeds), *policy_params)
    ):
        policies = [*policies]
