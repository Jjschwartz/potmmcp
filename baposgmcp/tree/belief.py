...
from baposgmcp.parts import PolicyID
...
class HPSParticleBelief(BaseParticleBelief):
    ...
    def __init__(self, agent_policy_id_map: Dict[M.AgentID, List[PolicyID]]):
        super().__init__()
        self._particles: List[H.HistoryPolicyState] = []
        self._agent_policy_particles: Dict[
            M.AgentID, Dict[PolicyID, List[H.HistoryPolicyState]]
        ] = {}
    ...
    def sample(self) -> H.HistoryPolicyState:
        return random.choice(self._particles)

    def sample_k(self, k: int) -> Sequence[H.HistoryPolicyState]:
        return random.choices(self._particles, k=k)

    def sample_agent_policy(self,
                            agent_id: M.AgentID,
                            policy_id: PolicyID) -> H.HistoryPolicyState:
    ...
    def sample_k_correlated(self, k: int) -> Sequence[H.HistoryPolicyState]:
    ...
    def add_particle(self, state: H.HistoryPolicyState):
    ...
    def get_dist(self) -> Dict[H.HistoryPolicyState, float]:
