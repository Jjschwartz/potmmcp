...
if __name__ == "__main__":
    ...
    for k in range(args.k):
        ...
        trainer_lk_remote = ray.remote(
            num_cpus=num_workers,
            num_gpus=num_gpus/args.k,
            memory=None,
            object_store_memory=None,
            resources=None
        )(PPOTrainer)

        trainer_lk = trainer_lk_remote.remote(
        ...
        igraph.add_policy(
            policy_lk_id, trainer_lk.get_weights.remote(policy_lk_id)
        )
    ...
    for i in range(args.stop_iters):
        ...
        for k in range(args.k):
            print(f"-- Level {k} --")
            result_futures.append(trainers[k].train.remote())

        results = ray.get(result_futures)
        for k in range(args.k):
            print(f"-- Level {k} --")
            print(pretty_print(results[k]))

            policy_lk_id = f"pi_{k}"
            igraph.update_policy(
                policy_lk_id, trainers[k].get_weights.remote(policy_lk_id)
            )
        ...
        for k in range(args.k):
            ...
            trainers[k].set_weights.remote(opp_weights)
