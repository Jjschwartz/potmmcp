...
This script only runs policies that were not run in the original experiment
script.
...
def get_baselines(agent_id: int, other_agent_id: int):   # noqa
    ...
    # with truncated, pucb and softmax and uniform meta-pi
    # = (2) + (5 * 2) + (5 * 1 * 1 * 2)
    # = 22
    exp_num_params = (
        len(meta_pis)
...
def main(args):   # noqa
    ...
    for (i, j) in [(0, 1), (1, 0)]:
        ...
        policy_params = get_baselines(i, j)
